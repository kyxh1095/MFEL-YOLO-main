...
class BaseModel(torch.nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, CE_Head):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            m.stride = fn(m.stride)
            m.anchors = fn(m.anchors)
            m.strides = fn(m.strides)
        return self
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):  # model, input channels, number of classes
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, ch, s, s).to(torch.device("cuda:0")))])  # forward
            self.stride = m.stride
            m.bias_init()  # only run once
        elif isinstance(m, CE_Head):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
                s = 640  # 2x min stride
                m.inplace = self.inplace
                m.cuda()
                forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
                m.stride = torch.tensor([s / x.shape[-2] for x in forward(torch.zeros(1, ch, s, s).to(torch.device("cuda:0")))])  # forward .to(torch.device("cuda:0"))
                self.stride = m.stride
                m.bias_init()
        else:
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m is HGBlock:
                ...
                n = 1
        elif m is td_lhfusion:
            if len(f) == 2:
                args = [ch[f[0]], ch[f[1]], *args]
            else:
                args = [ch[f[0]], [ch[i] for i in f[1:]], *args] # [ch[f[1]],ch[f[2]]]
            c2 = ch[f[0]]
        elif m is bu_lhfusion:
            if len(f) == 2:
                args = [ch[f[1]], *args]
                c2 = ch[f[1]]
            else:
                args = [ch[f[-1]], *args,[ch[i] for i in f[1:]]]
                c2 = ch[f[-1]]
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,CE_Head}
        ):
